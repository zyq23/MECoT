...
* **Title:** MECoT: 基于多专家协同与内化思维链的心理健康对话大模型 (Multi-Expert Collaborative Thought Framework for Psychological Interventions)
* **Badges/Links:** [Repository](https://github.com/zyq23/MECoT)
* **Introduction (Nominalized, precise):** This repository contains the official implementation of the MECoT framework. By integrating Cognitive Behavioral Therapy (CBT) and Humanistic therapy, the framework constructs a Chair Agent (ChairChat) to mitigate attention drift and achieve multi-agent collaboration via an internalized Chain-of-Thought (CoT).
* **Environment & Setup:** RTX 3090, ChatGLM2-6B, bfloat16, LoRA.
* **Repository Organization:** (As derived from user data)
* CBT Expert Module
* Humanistic Expert Module
* Fusion & Decision Module


* **Quick Start (Training & Inference):**
* Data Preparation (CPsyCoun formatting).
* LoRA fine-tuning for specific paradigm knowledge.
* ChairChat inference (temperature $T$, repetition penalty 1.1).


* **Citation:** Placeholder for the paper.


5. **Refining the Text (Applying Constraints):**
* *Constraint: No prefatory clauses.* Start the response directly with the README content.
* *Constraint: Academic Syntax.* Instead of "我们做了一个模型", use "本项目提供MECoT框架的官方实现代码".
* *Constraint: Terminology.*
* CBT -> 认知行为疗法 (CBT)
* Humanistic -> 人本主义疗法
* unconditional positive regard -> 无条件接纳
* Socratic questioning -> 苏格拉底式提问
* internalized Chain-of-Thought (CoT) -> 内化思维链 (CoT)
* attention drift -> 注意力漂移
* multi-agent collaboration -> 多智能体协同


* *Constraint: Zero AI-Touch.* Avoid any fluff. Use direct, factual statements. Let the code structure speak for itself.
* *Markdown structure:*
